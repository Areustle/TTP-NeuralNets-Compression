...
# echo python phase0.py --data_dir=$DATA_DIR --pretrained_model_dir=$PRETRAINED_MODEL --method=$METHOD --scope=$METHOD --rate=$RATE --output_path=$PHASE0

echo python phase1.py --batch_size=$BATCH_SIZE --data_dir=$DATA_DIR --pretrained_model_dir=$PRETRAINED_MODEL --method=$METHOD --scope=$METHOD --rate=$RATE --phase_zero=$PHASE0 --output_path=$PHASE1 --train_epochs=$EPOCHS --exp_growth=False
